...
const OrdenesCompraMateriaPrima = () => {
  ...
  useEffect(() => {
    ...
    fetchMateriasPrimas();
    fetchProveedores(); // Obtener proveedores al cargar
  ...
  const fetchProveedores = async () => {
    try {
      ...
      setProveedores(response.data); // Guardar los proveedores
    } catch (error) {
  ...
  const fetchOrdenesCompra = async () => {
    try {
      ...
      setOrdenesCompra(response.data); // Guardar las órdenes de compra
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!formState.proveedor_id) newErrors.proveedor_id = 'Proveedor es requerido';
  ...
  const handleSaveOrdenCompraMateriaPrima = async () => {
    if (validateForm()) {
      try {
        const { orden_compra_id, proveedor_id, materia_prima_id, cantidad } = formState;

        if (editingOrdenCompraMateriaPrima) {
          await axios.put(`/orden-compra-materiaprima/${editingOrdenCompraMateriaPrima.orden_compra_id}`, {
            proveedor_id,
            materia_prima_id,
            cantidad,
          });
  ...
  const handleEditOrdenCompraMateriaPrima = (ordenCompraMateriaPrima) => {
    ...
    setFormState({
      ...
      proveedor_id: ordenCompraMateriaPrima.proveedor_id,
      materia_prima_id: ordenCompraMateriaPrima.materia_prima_id,
      cantidad: ordenCompraMateriaPrima.cantidad,
    });
  };

  const handleDeleteOrdenCompraMateriaPrima = async (orden_compra_id) => {
    try {
      await axios.delete(`/orden-compra-materiaprima/${orden_compra_id}`);
      fetchOrdenesCompraMateriaPrima();
    } catch (error) {
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-br from-gray-100 to-gray-300">
      ...
      <div className={`flex-1 transition-all duration-300 ease-in-out ${sidebarOpen ? 'md:ml-64' : ''}`}>
        ...
        <main className="flex-1 p-6 md:p-8">
          {/* Formulario para crear o editar orden de compra de materia prima */}
            ...
            <table className="min-w-full table-auto border-collapse border border-gray-400">
              ...
              <tbody className="bg-white divide-y divide-gray-400">
                {currentItems.map((ordenCompraMateriaPrima) => {
                  ...
                  return (
                    <tr key={ordenCompraMateriaPrima.orden_compra_id} className="hover:bg-gray-100 transition duration-200">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-700 border border-gray-400 whitespace-nowrap">
                        {ordenCompraMateriaPrima.proveedor}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700 border border-gray-400 whitespace-nowrap">
                        {ordenCompraMateriaPrima.materia_prima}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700 border border-gray-400 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 text-right text-sm border border-gray-400 whitespace-nowrap">
                        <div className="flex space-x-2 justify-end">
                          ...
                          <button
                            onClick={() => handleDeleteOrdenCompraMateriaPrima(ordenCompraMateriaPrima.orden_compra_id)}

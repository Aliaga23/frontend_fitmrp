...
const Categorias = () => {
  ...
  const createOrUpdateCategoria = async () => {
    if (validateForm()) {
      try {
        if (editingId) {
          await axios.put(`/categories/${editingId}`, formState);
        } else {
          await axios.post('/categories', formState);
        }
        fetchCategorias();
        resetForm();
        setAnimate(true);
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-br from-gray-100 to-gray-300">
      ...
      <div className={`flex-1 transition-all duration-300 ease-in-out ${sidebarOpen ? 'md:ml-64' : ''}`}>
        <header className="flex justify-between items-center bg-gradient-to-r from-gray-900 to-gray-800 text-white p-6 shadow-md relative">
          <h1 className="text-3xl font-semibold">Gestión de Categorías</h1>
        ...
        <main className="flex-1 p-6 md:p-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg mb-8">
            <h3 className="text-2xl font-semibold text-gray-700 mb-6">Crear/Actualizar Categoría</h3>
  ...
  <table className="min-w-full table-auto border-collapse border border-gray-400">
    ...
    <tbody className="bg-white divide-y divide-gray-400">
      {currentItems.map((categoria, index) => (
        <tr key={categoria.id} className="hover:bg-gray-100 transition duration-200">
          <td className="px-6 py-4 text-sm text-gray-700 border border-gray-400">{index + 1 + (currentPage - 1) * itemsPerPage}</td>
          <td className="px-6 py-4 text-sm text-gray-700 border border-gray-400">{categoria.nombre}</td>
          ...
          {showModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
                ...
                <p className="mb-6">¿Estás seguro de que deseas eliminar esta categoría?</p>
